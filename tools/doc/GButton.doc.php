...
$GButtonDoc->setHeader(array())
		   ...
		   ->setConstant("IMAGE_24", array("button using an image of size 24"))
		   ->setConstant("IMAGE_32", array("button using an image of size 32"))
		   ->setConstant("IMAGE_48", array("button using an image of size 48"))
		   ->setMethod  ("__construct", array("Creates a GButton widget with a GLabel child containing the given text."))
		   ->setProperty("alwaysShowImage", array("@var boolean If TRUE, the button will ignore the “gtk-button-images” setting and always show the image, if available. Use this property if the button would be useless or hard to use without the image."))
		   ->setProperty("image", array("@var GWidget The child widget to appear next to the button text."))
