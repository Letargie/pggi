...
GTREE_VIEW_METHOD(on){
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "lz|z", "lz|z" ,&val ,&function, &param) == FAILURE) {
        RETURN_NULL();
    }
...
void gtree_view_init(int module_number){
	...
	le_gtree_view = zend_register_list_destructors_ex(gwidget_free_resource, NULL, "gtree view", module_number);
	...
	INIT_CLASS_ENTRY(ce, "GTreeView", gtree_view_class_functions);
