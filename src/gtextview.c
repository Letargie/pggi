...
GTEXT_VIEW_METHOD(__construct){
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "z", &textbuffer) == FAILURE) {
        RETURN_NULL();
    }
...
void gtext_view_init(int module_number){
	...
	le_gtext_view = zend_register_list_destructors_ex(gwidget_free_resource, NULL, "gwindow", module_number);
	...
	INIT_CLASS_ENTRY(ce, "GTextView", gtext_view_class_functions);
